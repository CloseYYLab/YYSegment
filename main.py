...
def train():
    ...
    if hp.mode == '2d':
        ...
        model = MiniSeg(in_input=hp.in_class + 2, classes=hp.out_class + 1)
    ...
    for epoch in range(1, epochs + 1):
        ...
        pbar = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{hp.total_epochs}",mininterval=0.3)
        ...
        for i, batch in enumerate(train_loader):
            ...
            if hp.mode == '2d':
                model_output_one_hot = torch.nn.functional.one_hot(labels, num_classes=hp.out_class + 1).permute(0, 3,
                                                                                                                 1, 2
                                                                                                                 )
            elif hp.mode == '3d':
                model_output_one_hot = torch.nn.functional.one_hot(labels, num_classes=hp.out_class + 1).permute(0, 4,
                                                                                                                 1, 2,
                                                                                                                 3)
            ...
            if hp.mode == '2d':
                y_one_hot = torch.nn.functional.one_hot(y_argmax, num_classes=hp.out_class + 1).permute(0, 3, 1, 2)
            if hp.mode == '3d':
                y_one_hot = torch.nn.functional.one_hot(y_argmax, num_classes=hp.out_class + 1).permute(0, 4, 1, 2, 3)

            false_positive_rate, false_negtive_rate, dice = metric(y_one_hot[:, 1:, :, :].cpu(),
                                                                   model_output_one_hot[:, 1:, :, :].cpu())
            ...
            #train_loader.desc = "train epoch[{}/{}] loss:{:.3f} lr:{:.3f}".format(epoch + 1,
                                                                               #epochs,
                                                                               #loss.item(), scheduler._last_lr[0])
            ...
            with torch.no_grad():
                ...
                if (hp.in_class == 1) and (hp.out_class == 1):
                    ...
                    output_image_artery.save(
                        os.path.join(args.output_dir, f"step-{epoch:04d}-predict_artery" + hp.save_arch))
                    ...
                    output_image_lung.save(
                        os.path.join(args.output_dir, f"step-{epoch:04d}-predict_lung" + hp.save_arch))

                    label_image_trachea = torchio.ScalarImage(tensor=y[2], affine=affine)
                    label_image_trachea.save(
                        os.path.join(args.output_dir, f"step-{epoch:04d}-gt_trachea" + hp.save_arch))

                    output_image_trachea = torchio.ScalarImage(tensor=outputs[2], affine=affine)
                    output_image_trachea.save(
                        os.path.join(args.output_dir, f"step-{epoch:04d}-predict_trachea" + hp.save_arch))
                    ...
                    output_image_vein.save(
                        os.path.join(args.output_dir, f"step-{epoch:04d}-predict_vein" + hp.save_arch))
